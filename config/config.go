...
// SetRateLimited marks the session as rate-limited with the given cooldown duration
func (s *SessionInfo) SetRateLimited(duration time.Duration) {
    s.mu.Lock()
    defer s.mu.Unlock()
    s.IsRateLimited = true
    s.RateLimitExpiry = time.Now().Add(duration)
}

// IsAvailable checks if the session is available for use
func (s *SessionInfo) IsAvailable() bool {
    s.mu.RLock()
    defer s.mu.RUnlock()
    if !s.IsRateLimited {
        return true
    }
    if time.Now().After(s.RateLimitExpiry) {
        s.mu.RUnlock()
        s.mu.Lock()
        s.IsRateLimited = false // Reset after expiry
        s.mu.Unlock()
        s.mu.RLock()
        return true
    }
...
type Config struct {
    ...
    IsMaxSubscribe         bool
    RateLimitCooldown      time.Duration // Default cooldown for rate limits
}
...
func LoadConfig() *Config {
    ...
    // Parse rate limit cooldown
    cooldown := os.Getenv("RATE_LIMIT_COOLDOWN")
    if cooldown != "" {
        if seconds, err := strconv.Atoi(cooldown); err == nil {
            config.RateLimitCooldown = time.Duration(seconds) * time.Second
        } else {
            config.RateLimitCooldown = 60 * time.Second
        }
    } else {
        config.RateLimitCooldown = 60 * time.Second
    }
...
func init() {
    ...
    logger.Info(fmt.Sprintf("RateLimitCooldown: %v", ConfigInstance.RateLimitCooldown))
}
