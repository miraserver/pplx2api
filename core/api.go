...
type Client struct {
    ...
    Attachments  []string
    OpenSerch    bool
    SessionInfo  *config.SessionInfo // Reference to session for rate limit tracking
}
...
func NewClient(sessionToken string, proxy string, model string, openSerch bool, sessionInfo *config.SessionInfo) *Client {
...
func (c *Client) SendMessage(message string, stream bool, is_incognito bool, gc *gin.Context) (int, error) {
    ...
    if resp.StatusCode == http.StatusTooManyRequests {
        ...
        cooldown := config.ConfigInstance.RateLimitCooldown // Use configured default
        ...
        // Mark session as rate-limited if we have session info
        if c.SessionInfo != nil {
            c.SessionInfo.SetRateLimited(cooldown)
            logger.Info(fmt.Sprintf("Rate limit hit for session %s, cooldown: %v", 
                c.SessionInfo.SessionKey[:8], cooldown))
        }
        
        resp.Body.Close()
        return http.StatusTooManyRequests, fmt.Errorf("rate limit exceeded, retry after %v", cooldown)
    }
